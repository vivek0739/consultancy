...
class Consultant_form extends CI_Model
{
	...
	function insertMTemp($sr_no,$modv)
	{
		...
		$query=$this->db->query("DELETE FROM consultant_member_temp  Where sr_no='$sr_no';");
		...
		{
			$this->db->insert('consultant_member_temp',$data);
		}
	}
	function insertRMTemp($sr_no)
	{
		...
		$query = $this->db->where('sr_no',$sr_no)->get('consultant_member_temp');
		$query1=$query->result();
	...
	function add_action($sr_no,$remark,$status,$auth,$date)
	{
		...
		$query=$this->db->query("INSERT INTO consultancy_form_track values('$sr_no','$remark','$status','$auth','$date')");
