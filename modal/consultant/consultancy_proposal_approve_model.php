...
class Consultancy_proposal_approve_model extends CI_Model
{
	...
	function get_default($sr_no,$payment_no)
	{
		...
		$query=("SELECT client_details.*,consultancy_proposal_details.*,consultancy_testing_type.*,consultancy_client_type.*
					FROM client_details
					INNER JOIN consultancy_proposal_details
					 ON client_details.sr_no = consultancy_proposal_details.sr_no
	...
	function get_default_prev($sr_no,$payment_no,$modification_value='')
	{
		...
		{
			$query=("SELECT client_detail_modification.*,consultancy_proposal_detail_modification.*,
						consultancy_testing_type_modification.*,consultancy_client_type_modification.*
					FROM client_detail_modification
					INNER JOIN consultancy_proposal_detail_modification
					 ON client_detail_modification.sr_no = consultancy_proposal_detail_modification.sr_no
					 AND client_detail_modification.modification_value = consultancy_proposal_detail_modification.modification_value
	...
	{
		$this->db->query("UPDATE consultancy_proposal_details set status='$status' where sr_no='$sr_no';");
	...
	function get_all_payment()
	{
		$query=("SELECT consultancy_charges.*,consultancy_proposal_details.*
					...
					ORDER BY consultancy_proposal_details.sr_no desc ,
					consultancy_charges.sr_no desc ;");
		$query1=$this->db->query($query);
		return $query1->result();
