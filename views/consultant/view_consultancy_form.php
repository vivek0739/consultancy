...
<?php
					...
					foreach($cons_row as $key => $cons_row) 
					{
						...
						if(($cons_row->status<5 && $cons_row->status!=1
							&& ($auth_id=='ft'||$auth_id=='c_i'||$auth_id=='hod'||$auth_id=='hos'))
							||($cons_row->status<5 && $cons_row->status>2 && ($auth_id=='pce') )
							||($cons_row->status<5 && $cons_row->status>3 && ($auth_id=='dt') ))
						{
				...
				?>
									...
									<td><i class="fa fa-link" style="cursor:pointer; color:#C00;" onclick='myFunction2("<?= $cons_row->sr_no ?>","<?= $auth_id?>")'></i>
												...
												<td align="center">
														...
														<?=$ui->button()->icon($ui->icon('check'))->mini()->extras('onclick = "myFunction5('.$cons_row->sr_no.')"')->uiType('success')->value('Done')->show();?>
...
function myFunction5(sr_no) {
    x = confirm("Are You Sure!");
    if(x == true)
    {
    	window.location = "<?php echo base_url().'index.php/consultant/consultancy_proposal_form/done/';?>"+sr_no
