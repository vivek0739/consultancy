	...
	public function index($sr_no='',$auth_id='')
	{
		...
		else
		{
			...
			if($upload)
            {
		            ...
		            //notification 
		            $description1= $this->session->userdata("id")." has submitted proposal form wiht payment no . ".
...
function done($sr_no)
{
	...
	$this->session->set_flashdata('flashSuccess','disbursement sheet for this consultancy will be generataed.');
	redirect('home');	        	
}
function edit($sr_no='',$payment_no='',$auth_id='')
	{
		...
		else
		{
			...
			$this->consultancy_proposal_form_model->update($data1,$data4,$data5,$data6,$payment_no);
           	$status=$payment_no*4+4;
			$this->add_action($data4['sr_no'],$this->session->userdata('id') .
					' has edited the proposal form with payment_no '.$payment_no,$status,'c_i' );
